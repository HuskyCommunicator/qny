...
def get_chat_stats(
    ...
    most_used_roles = [
        ...
        for stat in sorted(role_stats, key=lambda x: x.message_count, reverse=True)[:5]
    ]

    # 最近活动
    recent_sessions = db.query(ChatSession).filter(
        ChatSession.user_id == current_user.id
    ).order_by(desc(ChatSession.last_message_at)).limit(5).all()
    ...
    recent_activity = [
        ...
        for session in recent_sessions if session.last_message_at
    ]
